...
def cadastrar_locacao(driver):
    cliente_cadastrar_link = driver.find_element("css selector", 'a[href*="/locacoes/cadastrar"]')
    cliente_cadastrar_link.click()
...
def preencher_locacao(driver):
    ...
    try:
        ...
        print('input de teste: '+data+'\n')
        # Localizar os campos de entrada e preencher os dados

        try:
            campo_cidade = driver.find_element("id", "dataNascimento")
            campo_cidade.send_keys(data+'1212')
        except:
            resposta.append('Falhou no campo Data Nascimento: '+data)

        try:
            # Selecionar aleatoriamente o campo "Cliente"
            cliente_dropdown = driver.find_element("id", "cliente")
            clientes_options = cliente_dropdown.find_elements("tag name", "option")
            selected_cliente = random.choice(clientes_options)
            selected_cliente.click()
            
            # Selecionar aleatoriamente o campo "Locadora"
            locadora_dropdown = driver.find_element("id", "locadora")
            locadoras_options = locadora_dropdown.find_elements("tag name", "option")
            selected_locadora = random.choice(locadoras_options)
            selected_locadora.click()
...
def preencher_locadora_errado(driver):
    import Valid_Generator as vg
    ...
    try:
        #gerar tuplas
        nome= vg.generate_valid_phone()
        cidade= vg.generate_valid_phone()
        telefone= vg.generate_valid_password()
        cnpj= vg.generate_valid_password()
        senha= vg.generate_valid_phone()
        email= vg.generate_valid_phone()
    ...
    return entrada, resposta
def preencher_cliente_errado(driver):
    import Valid_Generator as vg
    ...
    try:
        #gerar tuplas
        nome= vg.generate_valid_phone()
        data= vg.generate_valid_password()
        telefone= vg.generate_valid_email()
        cpf= vg.generate_valid_password()
        senha= vg.generate_valid_cpf()
        email= vg.generate_valid_password()
    ...
    return entrada, resposta
def preencher_locacao_errado(driver):
    import Valid_Generator as vg
